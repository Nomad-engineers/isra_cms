...
There are many [Field Types](https://payloadcms.com/docs/fields/overview#field-types) Field Types to choose from, ranging anywhere from simple text strings to nested arrays and blocks. Most fields save data to the database, while others are strictly presentational. Fields can have [Custom Validations](https://payloadcms.com/docs/fields/overview#validation) Custom Validations, [Conditional Logic](https://payloadcms.com/docs/fields/overview#conditional-logic) Conditional Logic, [Access Control](https://payloadcms.com/docs/fields/overview#field-level-access-control) Access Control, [Hooks](https://payloadcms.com/docs/fields/overview#field-level-hooks) Hooks, and so much more.
...
To configure fields, use the `fields` property in your [Collection](https://payloadcms.com/docs/configuration/collections) Collection or [Global](https://payloadcms.com/docs/configuration/globals) Global config:
...
To begin writing fields, first determine which [Field Type](https://payloadcms.com/docs/fields/overview#field-types) Field Type best supports your application. Then author your field accordingly using the [Field Options](https://payloadcms.com/docs/fields/overview#field-options) Field Options for your chosen field type.
...
When `virtual` is set to `true`, the field becomes virtual but doesn't automatically populate any data. You'll typically use [Field-level Hooks](https://payloadcms.com/docs/fields/overview#field-level-hooks) Field-level Hooks to compute and populate the field's value:
...
All [Data Fields](https://payloadcms.com/docs/fields/overview#data-fields) Data Fields require a `name` property. This is the key that will be used to store and retrieve the field's value in the database. This property must be unique amongst this field's siblings.
...
In addition to being able to define [Hooks](https://payloadcms.com/docs/hooks/overview) Hooks on a document-level, you can define extremely granular logic field-by-field.
...
For full details on Field-level Hooks, see the [Field Hooks](https://payloadcms.com/docs/hooks/fields) Field Hooks documentation.

### Field-level Access Control

In addition to being able to define [Access Control](https://payloadcms.com/docs/access-control/overview) Access Control on a document-level, you can define extremely granular permissions field-by-field.
...
For full details on Field-level Access Control, see the [Field Access Control](https://payloadcms.com/docs/access-control/fields) Field Access Control documentation.
...
Default values can be defined as a static value or a function that returns a value. When a `defaultValue` is defined statically, Payload's [Database Adapters](https://payloadcms.com/docs/database/overview) Database Adapters will apply it to the database schema or models.
...
Fields are automatically validated based on their [Field Type](https://payloadcms.com/docs/fields/overview#field-types) Field Type and other [Field Options](https://payloadcms.com/docs/fields/overview#field-options) Field Options such as `required` or `min` and `max` value constraints. If needed, however, field validations can be customized or entirely replaced by providing your own custom validation functions.
...
| `event` | Either `onChange` or `submit` depending on the current action. Used as a performance opt-in. [More details](https://payloadcms.com/docs/fields/overview#validation-performance). |
...
For more performance tips, see the [Performance documentation](https://payloadcms.com/docs/performance/overview) Performance documentation.

## Custom ID Fields

All [Collections](https://payloadcms.com/docs/configuration/collections) Collections automatically generate their own ID field. If needed, you can override this behavior by providing an explicit ID field to your config. This field should either be required or have a hook to generate the ID dynamically.
...
**Reminder:** The Custom ID Fields can only be of type [Number](https://payloadcms.com/docs/fields/number) `Number` or [Text](https://payloadcms.com/docs/fields/text) `Text`. Custom ID fields with type `text` must not contain `/` or `.` characters.
...
| `components` | All Field Components can be swapped out for [Custom Components](https://payloadcms.com/docs/custom-components/overview) Custom Components that you define. |
...
| `style` | [CSS Properties](https://developer.mozilla.org/en-US/docs/Web/CSS) CSS Properties to inject into the root element of the field. |
| `className` | Attach a [CSS class attribute](https://developer.mozilla.org/en-US/docs/Web/CSS/Class_selectors) CSS class attribute to the root DOM element of a field. |
...
**Reminder:** To replace the Field Description with a [Custom Component](https://payloadcms.com/docs/custom-components/overview) Custom Component, use the `admin.components.Description` property. [More details](https://payloadcms.com/docs/fields/overview#description).

#### Description Functions

Custom Descriptions can also be defined as a function. Description Functions are executed on the server and can be used to format simple descriptions based on the user's current [Locale](https://payloadcms.com/docs/configuration/localization) Locale.
...
| `Filter` | The filter component rendered of the List View. [More details](https://payloadcms.com/docs/fields/overview#filter). |
...
For details on how to build Custom Components, see [Building Custom Components](https://payloadcms.com/docs/custom-components/overview#building-custom-components) Building Custom Components.

Instead of replacing the entire Field Component, you can alternately replace or slot-in only specific parts by using the [Label](https://payloadcms.com/docs/fields/overview#label) `Label`, [Error](https://payloadcms.com/docs/fields/overview#error) `Error`, [beforeInput](https://payloadcms.com/docs/fields/overview#afterinput-and-beforinput) `beforeInput`, and [afterInput](https://payloadcms.com/docs/fields/overview#afterinput-and-beforinput) `afterInput` properties.
...
| Property | Description |
|----------|-------------|
| `docPreferences` | An object that contains the [Preferences](https://payloadcms.com/docs/admin/preferences) Preferences for the document. |
...
| `i18n` | The [i18n](https://payloadcms.com/docs/configuration/i18n) i18n object. |
| `payload` | The [Payload](https://payloadcms.com/docs/local-api/overview) Payload class. |
...
To do so, import the [useField](https://payloadcms.com/docs/admin/react-hooks#usefield) `useField` hook from `@payloadcms/ui` and use it to manage the field's value:
...
For a complete list of all available React hooks, see the [Payload React Hooks](https://payloadcms.com/docs/admin/react-hooks) Payload React Hooks documentation. For additional help, see [Building Custom Components](https://payloadcms.com/docs/custom-components/overview#building-custom-components) Building Custom Components.
...
[Next: Array Field](https://payloadcms.com/docs/fields/array)